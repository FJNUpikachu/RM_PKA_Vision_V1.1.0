...
Dahua_CameraNode::Dahua_CameraNode(const rclcpp::NodeOptions &options):Node("camera_driver",options)
{
    ...
    camera_info_url_ = this->declare_parameter("camera_info_url", "package://rm_bringup/config/camera_info.yaml");
    ...
    RCLCPP_INFO(this->get_logger(),"DahuaCameraNode has been Start!");
}
...
void Dahua_CameraNode::timerCallback() 
{
    ...
    if (dt > 5.0) 
    {
        RCLCPP_WARN(this->get_logger(),"Camera is not alive! lost frame for %.2f seconds", dt);
        close();
    }
}
...
{
    RCLCPP_INFO(this->get_logger(),"Opening Dahua Camera Device!");
    if (is_open_) 
    {
        RCLCPP_WARN(this->get_logger(),"Dahua Camera Device is already opened!");
        close();
    }
    if(!is_open_)
    {
        ...
        {
            RCLCPP_WARN(this->get_logger(), "Camera parameters set failed!");
            return false;
        }
        ...
        {
            RCLCPP_WARN(this->get_logger(), "videoStart failed!!");
            return false;
        }
        // 先停止再启动
        if(!camera.stopGrabbing())
        {
            RCLCPP_WARN(this->get_logger(),"stopGrabbing failed!!");
            return false;
        } 
        // 开始拉流
        if(!camera.startGrabbing())
        {
            RCLCPP_WARN(this->get_logger(),"startGrabbing failed!!");
            return false;
        }
        RCLCPP_INFO(this->get_logger(), "Dahua Camera Device Open Success!");
...
{
    RCLCPP_INFO(this->get_logger(), "Closing Dahua Camera!");
    if (is_open_) 
    {
        // 断开拉流
        camera.stopGrabbing();
    ...
    RCLCPP_WARN(this->get_logger(), "Dahua Camera Device is already closed or No Camera!");
    return false;
...
bool Dahua_CameraNode::Check_connect() 
{
    ...
    while(1)
    {
        switch(connect_state)
        {
            ...
            {
                RCLCPP_WARN(this->get_logger(), "No camera connection found!");
                return false;
            }
            break;
            // 如果找到搜索到有相机连接则尝试连接相机
            ...
            {
                connect_state = 0;
                RCLCPP_WARN(this->get_logger(), "The camera connection failed!");
                //初始化相机失败次数达到设定值，强制退出
                return false;
            }
            break;
            default:
            //connect_state的初始值错误
            std::cout<<"The initial value of the connect_state is incorrect!"<<std::endl;
            return false;
        }
    }   
}
...
void Dahua_CameraNode::CaptureImage()  //取帧
{
    ...
    {   
        ...
        if(camera.getFrame(src))
        {
            ...
            {
                std::cout<<"Read Frame is empty!"<<std::endl;
                empty_frame++;
            }
        }
        else
        {
            RCLCPP_WARN(this->get_logger(),"GetFrame Failed!");
            empty_frame++;
        }  
        if(empty_frame>5)
        {
...
rcl_interfaces::msg::SetParametersResult Dahua_CameraNode::onSetParameters(std::vector<rclcpp::Parameter> parameters) 
{
    ...
    for (const auto &param : parameters) 
    {
        if (param.get_name() == "exposure_time") 
        {
            ...
            RCLCPP_INFO(this->get_logger(),"Set exposure_time:%lf", exposure_time_);
        } 
        else if (param.get_name() == "gain") 
        {
            gain_ = param.as_double();
            camera.SetAdjustPlus(gain_);
            RCLCPP_INFO(this->get_logger(),"Set gain:%lf", gain_);
        }
    }
    return result;
}
